...
com.modestmaps.PolygonMarker = function(map, locations, fillStyle, strokeStyle, lineWidth, lineJoin)
{
...
com.modestmaps.PolygonMarker.prototype = {
    ...
    draw: function(map)
    {
        ...
        this.div.style.display = 'block';
        this.div.style.left = point.x - (.5*buffer) + 'px';
        this.div.style.top = point.y - (.5*buffer) + 'px';
        ...
        if (this.drawZoom != map.getZoom()) {
            ...
            this.canvas.width = bottomRightPoint.x - topLeftPoint.x + buffer;
            this.canvas.height = bottomRightPoint.y - topLeftPoint.y + buffer;
        
            var points = [];
            for (var i = 0; i < this.coords.length; i++) {
                var point = map.coordinatePoint(this.coords[i]);
                point.x -= topLeftPoint.x - (.5*buffer);
                point.y -= topLeftPoint.y - (.5*buffer);
                points.push(point);
            }
